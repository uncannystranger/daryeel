...
export function SectionHeader({ eyebrow, title, accent, description, align = "center" }: SectionHeaderProps) {
  return (
    <div className={align === "center" ? "mx-auto max-w-3xl text-center" : "max-w-2xl"}>
      <p
        className={`mb-4 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.16em] text-[color:var(--accent)] ${
      ...
      <h2 className="text-balance text-[clamp(2rem,4vw,4rem)] font-black leading-[1.02] text-forest-950 dark:text-cream">
        {title}
        {accent ? <span className="text-forest-600 dark:text-[color:var(--accent)]"> {accent}</span> : null}
      </h2>
      {description ? (
